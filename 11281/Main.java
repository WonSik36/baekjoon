    ...
    public static void main(String[] args)throws IOException{
        ...
        if(result){
            ...
            for(int i=0;i<scc.length;i++){
                nodeList.add(new Node(i, scc[i]));
            }

            Collections.sort(nodeList, comparingInt((Node e) -> e.sccNum));
            ...
            for(Node n : nodeList){
                ...
                if(nodeValues[nodeNum / 2] != -1)
                    continue;
                nodeValues[nodeNum / 2] = nodeNum % 2 == 1 ? 0 : 1; // Not Node -> node: true, Node -> node: false
            }
...
class Node {
    ...
    public Node(int nodeNum, int sccNum){
